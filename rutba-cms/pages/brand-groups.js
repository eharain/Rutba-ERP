...
export default function BrandGroups() {
    ...
    const bulkPublish = async () => {
        ...
        if (!confirm(`Publish ${ids.length} brand group(s)?`)) return;
        let ok = 0, fail = 0;
        ...
        toast(`Published ${ok} brand group(s)${fail ? `, ${fail} failed` : ""}.`, fail ? "warning" : "success");
    ...
    const bulkUnpublish = async () => {
        const ids = [...selectedIds];
        if (ids.length === 0) { toast("No items selected.", "warning"); return; }
        if (!confirm(`Unpublish ${ids.length} brand group(s)?`)) return;
        let ok = 0, fail = 0;
        for (const docId of ids) {
            setPublishing(prev => ({ ...prev, [docId]: true }));
        ...
        toast(`Unpublished ${ok} brand group(s)${fail ? `, ${fail} failed` : ""}.`, fail ? "warning" : "success");
        setSelectedIds(new Set());
    };
