...
export default function OfferDetail() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            authApi.get(`/offers/${documentId}`, { status: 'draft', populate: ["product_groups", "cms_pages", "categories"] }),
            authApi.get(`/offers/${documentId}`, { status: 'published', fields: ["documentId"] }).catch(() => ({ data: null })),
    ...
    const handleSave = async () => {
        ...
        try {
            if (isNew) {
                const res = await authApi.post("/offers", buildPayload());
                const created = res.data || res;
                router.push(`/${created.documentId}/offer`);
            } else {
                await authApi.put(`/offers/${documentId}?status=draft`, buildPayload());
    ...
    const handlePublish = async () => {
        ...
        try {
            await authApi.put(`/offers/${documentId}?status=draft`, buildPayload());
            await authApi.post(`/offers/${documentId}/publish`, {});
            setIsPublished(true);
    ...
    const handleUnpublish = async () => {
        setSaving(true);
        try {
            await authApi.post(`/offers/${documentId}/unpublish`, {});
            setIsPublished(false);
            toast("Offer unpublished.", "success");
        } catch (err) {
            console.error("Failed to unpublish offer", err);
    ...
    const handleDelete = async () => {
        ...
        try {
            await authApi.del(`/offers/${documentId}`);
            router.push("/offers");
        } catch (err) {
        ...
        <ProtectedRoute>
            <Layout>
                ...
                <div className="d-flex align-items-center mb-3">
                    <Link className="btn btn-sm btn-outline-secondary me-3" href="/offers">
                        <i className="fas fa-arrow-left"></i> Back
                    </Link>
                    <h2 className="mb-0">{isNew ? "New Offer" : "Edit Offer"}</h2>
                    ...
                    <div className="ms-auto d-flex gap-2">
                        ...
                        <button className="btn btn-sm btn-primary" onClick={handleSave} disabled={saving}>
                            {saving ? "Saving…" : isNew ? "Create Offer" : "Save Draft"}
                        </button>
                        {!isNew && (
                ...
                {loading && <p>Loading...</p>}
                {!loading && !isNew && !offer && <div className="alert alert-warning">Offer not found.</div>}
