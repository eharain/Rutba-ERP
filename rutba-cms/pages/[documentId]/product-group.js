...
export default function ProductGroupDetail() {
    ...
    useEffect(() => {
        ...
        authApi.get(`/product-groups/${documentId}`, { populate: ["gallery", "cover_image", "products.logo", "products.brands", "products.categories"] })
            .then(res => {
    ...
    const loadPickerData = useCallback(async () => {
        ...
        try {
            const [productsRes, brandsRes, categoriesRes] = await Promise.all([
                authApi.get("/products", { pagination: { pageSize: 500 }, sort: ["name:asc"], populate: ["logo", "brands", "categories"] }),
                authApi.get("/brands", { pagination: { pageSize: 200 }, sort: ["name:asc"] }),
                authApi.get("/categories", { pagination: { pageSize: 200 }, sort: ["name:asc"] }),
            ]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isNew) {
                ...
                await authApi.put(`/product-groups/${documentId}`, payload);
                toast("Product group updated!", "success");
            }
        } catch (err) {
                                ...
                                <>
                                    ...
                                    <div className="card mb-3">
                                        ...
                                        <div className="card-body">
                                            ...
                                            {group.publishedAt && <span className="badge bg-success">Published</span>}
                                            {!group.publishedAt && <span className="badge bg-secondary">Draft</span>}
                                        </div>
                                    </div>
                                </>
                            )}
