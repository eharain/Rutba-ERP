...
export default function VideoStudioPage() {
    ...
    // ── room to work ────────────────────────────────────────
    // Focus promotes ONE area to the whole window; hiding the rail hands its
    // width to the stage. Both are STYLE on the containers that are already
    ...
    useEffect(() => {
        try { setRailHidden(localStorage.getItem(`${SETTINGS_KEY}:railHidden`) === "1"); } catch { /* private mode */ }
    }, []);
    const toggleRail = () => setRailHidden((v) => {
        const next = !v;
        try { localStorage.setItem(`${SETTINGS_KEY}:railHidden`, next ? "1" : "0"); } catch { /* private mode */ }
        return next;
    });
    // Escape leaves focus, and the page behind must not scroll under it.
    useEffect(() => {
    ...
    const focusStyle = (which) => (focus === which
        ? {
            position: "fixed", inset: 0, zIndex: 1080, width: "auto", maxWidth: "none",
            overflowY: "auto", padding: 12, background: "var(--bs-body-bg, #fff)",
        }
        : null);
            ...
            <Layout fullWidth>
                ...
                {selected && (
                    <div className="d-flex gap-3 align-items-start">
                        ...
                        <div className="flex-grow-1" style={{ minWidth: 0, ...focusStyle("stage") }}>
                                <div className="card">
                                    <div className="card-header py-2 d-flex align-items-center gap-2">
                                        ...
                                        <button className="btn btn-sm btn-outline-secondary"
                                            ...
                                            <i className={`fas ${focus === "stage" ? "fa-compress" : "fa-expand"}`} />
                                        </button>
                                        {focus !== "stage" && (
                                            <button className="btn btn-sm btn-outline-secondary"
                                                title={railHidden ? "Show the properties panel" : "Hide the properties panel — the stage takes its width"}
                                                onClick={toggleRail}>
                                                <i className={`fas ${railHidden ? "fa-angles-left" : "fa-angles-right"}`} />
                                            </button>
                                        )}
                                    </div>
                        ...
                        {/* ── the inspector: the SELECTED layer's properties, or the video's.
                               Sticky and full height, with its own scroll, so the rail and the
                               stage never fight over one page scrollbar. ── */}
                        <div className="flex-shrink-0" style={{
                            width: railHidden && !focus ? 0 : 340,
                            overflow: railHidden && !focus ? "hidden" : "visible",
                            position: focus ? "static" : "sticky",
                            top: 8,
                            ...focusStyle("rail"),
                        }}>
                            <div style={{
                                maxHeight: focus === "rail" ? "none" : "calc(100vh - 24px)",
                                overflowY: focus === "rail" ? "visible" : "auto",
                                paddingRight: 4,
                            }}>
                                <audio ref={trackAudioRef} className="d-none" onEnded={() => setPreviewingId(null)} />

                                <div className="d-flex align-items-center gap-1 mb-2">
                                    <small className="text-muted text-truncate flex-grow-1">Properties</small>
                                    <button className="btn btn-sm btn-outline-secondary py-0"
                                        title={focus === "rail" ? "Leave full screen (Esc)" : "Full screen — properties fill the window"}
                                        onClick={() => setFocus(focus === "rail" ? null : "rail")}>
                                        <i className={`fas ${focus === "rail" ? "fa-compress" : "fa-expand"}`} style={{ fontSize: 11 }} />
                                    </button>
                                    {!focus && (
                                        <button className="btn btn-sm btn-outline-secondary py-0" title="Hide this panel — the stage takes its width"
                                            onClick={toggleRail}>
                                            <i className="fas fa-angles-right" style={{ fontSize: 11 }} />
                                        </button>
                                    )}
                                </div>
