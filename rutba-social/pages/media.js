...
export default function MediaPage() {
    ...
    const [folderLayout, setFolderLayout] = useState('top'); // 'top' | 'left'
    ...
    const handleMoveToFolder = async (targetFolderId) => {
        const ids = selectedFileIds.size > 0 ? Array.from(selectedFileIds)
            : draggedFileIds.length > 0 ? [...draggedFileIds]
    ...
    const hasMovableSelection = selectedFileIds.size > 0 || draggedFileIds.length > 0 || selectedFile != null;
    ...
    return (
        <ProtectedRoute>
            <Layout fullWidth>
                ...
                <div className="card mb-3">
                    <div className="card-body py-2">
                        <div className="d-flex flex-wrap align-items-center gap-2">
                            ...
                            <button className={`btn btn-sm ${folderLayout === 'left' ? 'btn-warning' : 'btn-outline-warning'}`}
                                onClick={() => setFolderLayout(l => l === 'top' ? 'left' : 'top')}
                                title={folderLayout === 'top' ? 'Switch to sidebar folders' : 'Switch to top folders'}>
                                <i className={`fas ${folderLayout === 'top' ? 'fa-columns' : 'fa-grip-horizontal'}`} />
                            </button>
                ...
                <div className={folderLayout === 'left' ? 'd-flex' : ''} style={{ minHeight: '50vh' }}>
                    {/* Folder sidebar — left mode */}
                    {folderLayout === 'left' && (
                        <div className="border rounded-start bg-white me-0 d-flex flex-column" style={{ width: 230, minWidth: 230, padding: '8px 0' }}>
                ...
                {draggedFileIds.length > 0 && (
                    <div className="position-fixed bottom-0 start-50 translate-middle-x mb-3" style={{ zIndex: 9999 }}>
                        <div className="alert alert-info py-2 px-3 shadow d-flex align-items-center mb-0">
                            <i className="fas fa-arrows-alt me-2" /> Drop on a folder or click <i className="fas fa-paste mx-1" /> to move {draggedFileIds.length} file(s)
                        </div>
                    </div>
                )}
            </Layout>
