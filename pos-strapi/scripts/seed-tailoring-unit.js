...
/**
 * One-off demo/seed data for a tailoring & stitching unit.
 *
 * Boots Strapi programmatically (load only — no HTTP listen, safe to run while
 * the dev server is up) and creates a coherent dataset through the documents
 * API so components, relations and lifecycles (stock recompute) all fire:
 *
 ...
 * Aborts if mfg-operations already contain rows (no duplicate guard beyond that).
 */
...
async function main() {
  ...
  const create = async (uid, data) => {
    try {
      return await doc(uid).create({ data });
    } catch (err) {
      console.error(`CREATE FAILED ${uid}: ${err.message}`);
  ...
  if (existingOps > 0) {
    console.log(`mfg_operations already has ${existingOps} rows — aborting to avoid duplicates.`);
    await app.destroy();
    process.exit(2);
  }
  ...
  console.log('Products created (9 raw, 5 finished).');
  ...
  console.log('Employees created (14).');
  ...
  console.log('Production lines created (5).');
  ...
  console.log('Operations created (8).');
  ...
  console.log('Defect types created (10) and linked to operations.');
  ...
  console.log('Worker profiles created (13).');
  ...
  console.log(`Piece rates created (${rateDefs.length}).`);

  // ---- BOMs ------------------------------------------------------------
  const routing = (steps) => steps.map(([key, seq, mins, grade], i) => ({
  ...
  console.log('Material lots created (9).');
  ...
  // WO-4: Draft
  const wo4 = await create('api::mfg-work-order.mfg-work-order', {
  ...
  console.log('Work orders created (5).');

  // ---- bundles ------------------------------------------------------------
  const mkBundle = (data) => create('api::mfg-bundle.mfg-bundle', data);
  const b1s = await mkBundle({ bundle_code: 'B-0001-S', size: 'S', color: 'White', quantity: 10, quantity_completed: 10, status: 'Completed', current_operation_seq: 8, current_operation: op.packing.documentId, work_order: wo1.documentId, production_line: lineGents.documentId });
  const b1m = await mkBundle({ bundle_code: 'B-0001-M', size: 'M', color: 'White', quantity: 10, quantity_completed: 10, status: 'Completed', current_operation_seq: 8, current_operation: op.packing.documentId, work_order: wo1.documentId, production_line: lineGents.documentId });
  const b1l = await mkBundle({ bundle_code: 'B-0001-L', size: 'L', color: 'White', quantity: 10, quantity_completed: 8, quantity_rejected: 2, status: 'Completed', current_operation_seq: 8, current_operation: op.packing.documentId, work_order: wo1.documentId, production_line: lineGents.documentId });
  ...
  const b2c = await mkBundle({ bundle_code: 'LB-0002-3', size: 'M', color: 'Floral', quantity: 10, status: 'Issued', current_operation_seq: 1, current_operation: op.cutting.documentId, work_order: wo2.documentId, production_line: lineLadies.documentId });
  ...
  const b3a = await mkBundle({ bundle_code: 'KB-0003-1', size: 'M', color: 'Black', quantity: 13, status: 'Created', current_operation_seq: 0, work_order: wo3.documentId, production_line: lineGents.documentId });
  const b3b = await mkBundle({ bundle_code: 'KB-0003-2', size: 'L', color: 'Black', quantity: 12, status: 'Created', current_operation_seq: 0, work_order: wo3.documentId, production_line: lineGents.documentId });
  ...
  console.log('Bundles created (10).');
  ...
  console.log('Material issues created (9).');
  ...
  await approved('shahid', 'stitching', wo1, b1s, lineGents, 12, 12, 450, '2026-05-20T09:00:00+05:00', '2026-05-24T17:00:00+05:00');
  await approved('naveed', 'stitching', wo1, b1m, lineGents, 10, 10, 380, '2026-05-20T09:00:00+05:00', '2026-05-25T17:00:00+05:00', { rejected: 1, notes: '1 pc failed final QC (oil stain)' });
  await approved('zubair', 'stitching', wo1, b1l, lineGents, 8, 8, 320, '2026-05-20T09:00:00+05:00', '2026-05-26T17:00:00+05:00', { rejected: 1, notes: '1 pc failed final QC (open seam)' });
  ...
  console.log('QC inspections created (3).');
  ...
  console.log(`Finished stock-items created (${createdStock}).`);
  ...
  console.log(`product.stock_quantity for ${fg.gskWhite.sku} set to ${inStock}.`);
