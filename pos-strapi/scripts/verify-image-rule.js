 ...
 * rutba-marketplace and is covered by its own suite — `npm test` there, under
 * "catalog image gate".
 *
 ...
 *
 * Run:  node scripts/js/load-env.js -- npm --prefix pos-strapi run verify:image-rule
...
async function verifyPickerFilter(app, knex) {
  ...
  // post subject), drafts — exactly what rutba-social/pages/products.js sends.
