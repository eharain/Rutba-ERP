 ...
 * only covers one:
 *   - POST /products/:id/publish        (rutba-cms, single + bulk)
 *   - the Strapi admin content-manager Publish button (bypasses our routes)
 *   - any server-side documents().publish() call
 *   - rutba-core's /api/products/:id/publish, when RUTBA_BACKEND is core|both
 *     (registered separately in rutba-core/src/modules/catalog.js — core has
...
// Names the actual reason. rutba-cms surfaces the server message in its toast,
...
/**
 * Register the gate on a documents() implementation. Shared by pos-strapi's
 * register() and rutba-core's catalog module so the rule has one definition
