...
module.exports = factories.createCoreController(
    ...
    ({ strapi }) => ({
        ...
        async create(ctx) {
            ...
            let createdOrder;
            try {
                createdOrder = await strapi.documents('api::order.order').create({
                    data: orderData,
            ...
            if (selectedDeliveryMethod?.service_provider === 'easypost' && easypost_rate_id) {
                try {
                    ...
                    createdOrder = await strapi.documents('api::order.order').update({
        ...
        async webhookStripe(ctx) {
            ...
            await strapi.db.query('api::order.order').update({ where: { order_id: orderId }, data: updateData });
        ...
        async cancelOrder(ctx) {
            ...
            const order = await strapi.documents('api::order.order').findOne({ documentId, populate: ['owners'] });
            if (!order) return ctx.notFound('Order not found');
