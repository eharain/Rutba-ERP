...
in the CRM app's path, and moved the shared component to where the convention
says it lives:

- `EnumSelect` + `use-enum-values` moved `rutba-cms` → **`pos-shared`**
  (`components/EnumSelect.js`, `hooks/use-enum-values.js`); rutba-cms's three
...
## Handoff to `rutba-campaigns` (roadmap 1.4)

That build has started. Everything it needs from CRM is here, with two
things it must do on its side:

1. **Register its domain, then widen the descriptor.** `crm-segments.js`
   currently declares `apps: ['crm']` on every method. Once the app has its
   entry in `config/domains.json` + `roles.js` (the new-ERP-app checklist),
   add that domain to `list`, `byId` and `listAudience` — a campaigns-domain
   caller gets a 403 until then, and the failure is a silent 403 rather than
   anything that points at this file. Re-run
   `npm run seed -- --only=api-provider,up-permissions` after.
2. **Consume `listAudience`, not `listMembers`.** See the comparison table
   above. `listMembers` is the report grid; using it as a send list mails the
   same person once per lead they have.
