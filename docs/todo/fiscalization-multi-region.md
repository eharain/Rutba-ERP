...
- Per repo convention, any of this that lands later needs an api-provider descriptor + `node ./scripts/scaffold-endpoint-providers.mjs` + the three validators; custom controller actions ported into `services/core/src/modules/*.js`; and reference data as a seeder in `services/strapi/src/seed/registry.js` with a matching migration.
...
- [tax-profiles.js](../../services/strapi/src/seed/seeders/tax-profiles.js) — the existing multi-region rate layer
