...
Also fixed while writing the seed: `recordAudit` de-duplicates on unresolved
rows, so neither the backfill re-running nor the controller firing on every
edit can grow duplicate audit entries for the same finding.
