...
- `rutba-core/src/auth/up.js` — the users-permissions subsystem: session
  manager, user service, plugin-store reader.
- `rutba-core/src/modules/auth.js` — the endpoint surface + the pos-strapi
...
Tranches 1–7 `require()` pos-strapi source against the compat strapi. The auth
...
re-implements the CONTRACT while still loading, from pos-strapi's own
...
`UP_*_LIFESPAN` env vars pos-strapi's config uses.
...
### pos-strapi extension behaviours reproduced (repo code the apps depend on)

- register grants the `web_user` app-role to `authenticated`-role signups,
...
pos-strapi by `scripts/check-cross-server-auth.js` (12 checks, all green):

- a **core**-minted access token authenticates on **pos-strapi**,
- a **pos-strapi**-minted access token authenticates on **core**,
- pos-strapi logs in a user **core** registered (same hash recipe),
- **core** rotates a refresh token **pos-strapi** issued, and the result is
  accepted by pos-strapi,
- a core-issued refresh token rotates into a token pos-strapi accepts,
...
2. **Email.** `forgot-password` and `send-email-confirmation` record the token
   ...
   pos-strapi to send the link, or the token must be read from the row.
...
`POST /api/auth/refresh` on **pos-strapi 403s for every caller** on the dev
...
pos-strapi. Core does not reproduce this. Fix belongs in the up-permissions
...
- `node scripts/check-cross-server-auth.js` — 12 checks, needs pos-strapi up.
