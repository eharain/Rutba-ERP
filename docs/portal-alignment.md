...
| **E3** portal auth | **seam built, readers moved, both doors verified, sync-up built.** The seam answers at two depths — `subjectOf(ctx)` (synchronous, no database) and `identityOf(ctx)` (the same plus role claims); both api-pro policy gates, the upload gate, the request logger and helpdesk's audit correlation id read it, so a portal-authenticated caller is policed as a person instead of skipping every check. What still reads `ctx.state.user` is the local door itself (`/me/permissions`, sessions/password/me, helpdesk's actor row), each marked in place. **Inbound:** `src/platform/assertion.js` verifies the gateway's `X-Rutba-Assertion` against the frozen `@rutba/contracts` shape — internal JWKS and never auth's keys (so an edge token pushed at an instance fails `unknown_key`), 120s cap, single-audience, `jti` replay window, algorithm from the key. **Outbound:** `src/platform/scim.js` builds membership writes per GLOBAL-AUTH.md §5 — create/link, role replace, deactivate — and refuses locally to build anything central owns (`userName`, name, emails, credentials), because a `mutability` refusal discovered on the wire is an edit the person who typed it never learns was dropped. **Login:** `@rutba/shared/core/auth` holds PKCE (checked against RFC 7636's own vector), the `org_hint` authorization request, discovery, code exchange and edge-token verification — in `packages/shared` because the app that needs it is a Next app, and sharing one JWS verifier with the instance rather than keeping two that drift. Four contract tests, 121 checks, all against the shared signed fixtures: `smoke:identity`, `smoke:assertion`, `smoke:scim`, `smoke:oidc`. **Every door is shut until configured** — `RUTBA_GATEWAY_*`/`RUTBA_SERVICE_IDENTITY` for assertions, `RUTBA_SCIM_*` for sync-up — and an assertion at an unconfigured instance is a loud 501, never an ignored header. **Open:** the Next login routes in `apps/admin/auth` (redirect, callback, session cookie) and wiring SCIM into HR's write paths. Both need auth.rutba.io to exist — it is still an empty scaffold — and a login flow nobody can exercise would pass its own tests and prove nothing. |
...
2. **Consume the platform client packages** (published from Rutba-Portal, plan 00): `@rutba/portal-auth` (JWT/JWKS verification + claims mapping), `@rutba/license-client` (validate/heartbeat, entitlement schema, last-known-good cache), `@rutba/usage-reporter` (idempotent usage events). Until they're published, implement behind the same interfaces so the swap is mechanical. **Done for auth:** `@rutba/shared/core/auth` implements the `@rutba/portal-auth` surface — one JWS verifier for all three token layers, the OIDC client, and PKCE — against `@rutba/contracts` fixtures rather than against guesses. That package is unpublished, so the fixtures are read by path (`services/core/scripts/lib/contracts.js`) and the tests skip loudly when the platform repo is absent; a `file:` dependency is what once dropped the `api_pro_*` tables.
