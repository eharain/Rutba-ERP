...
| **E3** portal auth | **seam built, door shut.** `identityOf(ctx)` answers one `{sub, org_id, roles, entitlements, req_id, source}` for local JWT / API token / (future) portal assertion, translating role keys into the portal's `domain:level` claims — split on the LAST underscore, checked against all 76 keys. No JWKS verification: an unconfigured portal refuses rather than falling back to local verification. OIDC client shell, SCIM, and moving readers off `ctx.state.user` are still open. |
