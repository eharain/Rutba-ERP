...
export default function Grid4Layout({ group, sort = "default", offerActive }: Grid4LayoutProps) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
      {products.map((item) => (
        <ProductCard key={"g4-" + item.id} {...getProductCardProps(item, { showBrand: group.show_brand, showCategory: group.show_category, offerActive })} />
      ))}
    </div>
  );
}
