...
const VIDEO_PROVIDERS = [
    {
        ...
        pattern: /^https?:\/\/(?:www\.)?(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/)|youtu\.be\/)([\w-]+)/,
    ...
    {
        // Facebook video  – facebook.com/watch/?v=ID  |  fb.watch/ID
        pattern: /^https?:\/\/(?:www\.)?(?:facebook\.com\/(?:watch\/?\?v=|[\w.]+\/videos\/)|fb\.watch\/)([\w-]+)/,
        embed: (_id, url) =>
            `https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(url)}&show_text=false`,
    },
    {
        // Instagram reel / post  – instagram.com/reel/ID  |  instagram.com/p/ID
...
function buildIframe(src, maxWidth) {
    const widthStyle = maxWidth && maxWidth !== '100%' ? `max-width:${maxWidth};margin-left:auto;margin-right:auto;` : 'max-width:100%;';
    return (
...
function buildTweetEmbed(src, maxWidth) {
...
function buildOverlayPlayer(embedSrc, overlayUrl, maxWidth) {
    const widthStyle = maxWidth && maxWidth !== '100%' ? `max-width:${maxWidth};margin-left:auto;margin-right:auto;` : 'max-width:100%;';
    const escapedSrc = escapeHtml(embedSrc);
    const escapedOverlay = escapeHtml(overlayUrl);

    // The onclick replaces the overlay with the iframe
    ...
    return {
        extensions: [
            // ::video[URL]{size="medium" overlay="..."}
            {
                ...
                renderer(token) {
                    const embedSrc = token.provider.embed(token.providerId, token.href);
            ...
            // Bare URL on its own line (backward-compatible)
            {
                ...
                renderer(token) {
                    const src = token.provider.embed(token.providerId, token.href);
                    if (token.provider.type === 'tweet') {
