...
import { PurchaseItemsEndpoints, savePurchaseItem } from './purchase-items.js';
...
export async function savePurchase(idx, purchase) {
    ...
    for (const item of items) {
        ...
        const savedItem = await savePurchaseItem(item);
 ...
 * @param {number} rowsPerPage
 */
export async function searchPurchases(searchTerm, page = 1, rowsPerPage = 5) {
    ...
    const res = await AuthApiEndpoints.fetch(`/purchases?${qs.stringify(query, { encodeValuesOnly: true })}`);
    return dataNode(res);
}
